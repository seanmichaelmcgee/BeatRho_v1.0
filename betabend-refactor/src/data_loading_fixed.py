...
def load_precomputed_features(
    ...
    with np.load(thermo_path) as data:
        ...
        if "positional_entropy" in data:
            ...
            if logger.isEnabledFor(logging.DEBUG):
                logger.debug(f"No positional entropy found for {target_id}. Using zeros.")
        ...
        if "accessibility" in data:
            ...
            if "logger" not in locals():
                ...
                logger = logging.getLogger(__name__)
            if logger.isEnabledFor(logging.DEBUG):
                logger.debug(f"No accessibility found for {target_id}. Using zeros.")
    ...
    if os.path.exists(mi_path):
        ...
        if "logger" not in locals():
            ...
            logger = logging.getLogger(__name__)
        if logger.isEnabledFor(logging.DEBUG):
            logger.debug(f"Evolutionary features not found for {target_id}. Using zeros.")
...
def collate_fn(batch: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    if not batch_size:
        return {}  # Return empty dictionary for empty batch
    ...
    if "meta" in batch[0]:
        ...
        for key in batch[0]["meta"].keys():
            try:
                ...
                logger.warning(f"Error stacking meta key {key}: {e}")
                # Store as list when stacking fails
                meta[key] = [sample["meta"][key] for sample in batch]
        output["meta"] = meta
    ...
    for key in batch[0].keys():
        ...
        if isinstance(batch[0][key], torch.Tensor):
            # Check if all tensors have the same shape after first dimension
            shapes = [sample[key].shape for sample in batch if key in sample]
            same_feature_dims = all(s[1:] == shapes[0][1:] for s in shapes) if shapes and len(shapes[0]) > 1 else True
            ...
            try:
                ...
                if len(sample_shape) == 1:
                    ...
                    if same_feature_dims:
                        # All samples have same feature dimension, proceed with stacking
                        output[key] = torch.stack(
                            [pad_2d(sample[key], max_len) for sample in batch]
                        )
                    else:
                        # Different feature dimensions, handle each sample separately
                        logger.warning(f"Inconsistent feature dimensions in key {key}: {[s[1] for s in shapes]}")
                        # Keep as list of tensors to avoid stacking error
                        output[key] = [pad_2d(sample[key], max_len) for sample in batch]
                    ...
                    if same_feature_dims:
                        ...
                        # Different feature dimensions, handle each sample separately
                        logger.warning(f"Inconsistent dimensions in key {key}: {shapes}")
                        # NOTE: We will need to handle these cases specially in the loss functions
                        # Keep track of device for consistency during later processing
                        device_info = batch[0][key].device if hasattr(batch[0][key], 'device') else None
                        logger.info(f"Creating list for inconsistent tensors with reference device: {device_info}")
                        
                        # NOTE: When we return a list, the model/loss must handle device placement
                        # Keep as list to avoid stacking error
                        output[key] = [pad_tensor(sample[key], (max_len,) + s[1:]) 
                                      for sample, s in zip(batch, shapes)]
            
            except RuntimeError as e:
                logger.warning(f"Error stacking key {key}: {e}")
                # Check if the error might be related to device issues
                if "device" in str(e).lower():
                    logger.error(f"Device-related error detected. This should be handled by the loss functions.")
                
                # Log device info for debugging
                sample_devices = [sample[key].device if hasattr(sample[key], 'device') else 'unknown' 
                                 for sample in batch if key in sample]
                if sample_devices:
                    logger.info(f"Devices in batch for {key}: {sample_devices[:3]}...")
                
                # Fallback: store as list instead of stacked tensor
                # NOTE: The model and loss functions must handle device placement when using lists!
                output[key] = [sample[key] for sample in batch if key in sample]

        elif isinstance(batch[0][key], (int, float)):
            # Handle scalar values
            output[key] = torch.tensor(
                [sample[key] for sample in batch if key in sample]
            )
