#!/bin/bash
# HIGH-PERFORMANCE PRODUCTION TRAINING WITH ENHANCED VALIDATION - V6
# VALIDATION-ENHANCED: Includes dual-mode validation with test-equivalent metrics
# MAXIMUM INTENSITY: ~3x resources of v4, ~15x of v3
# WARNING: This will use substantial GPU resources (target: ~50% of RTX 4070)
# Builds on the v5 training script with comprehensive validation integration
...
OUTPUT_DIR="results/production_run_validation_v6"
mkdir -p $OUTPUT_DIR
...
# Run comprehensive checkpoint validation using ValidationRunner
validate_checkpoint() {
    local checkpoint_path="$1"
    local validation_log="${OUTPUT_DIR}/checkpoint_validation.log"
    
    echo "Running comprehensive validation on checkpoint: $checkpoint_path" >> "$validation_log"
    
    # Create a timestamped validation directory
    local timestamp=$(date +%Y%m%d-%H%M%S)
    local validation_dir="${OUTPUT_DIR}/validation_results/${timestamp}"
    mkdir -p "$validation_dir"
    
    # Run our enhanced validation script that uses ValidationRunner
    echo "Executing dual-mode validation (test-equivalent and training-equivalent)..." >> "$validation_log"
    python scripts/validate_enhanced_model.py \
        --checkpoint "$checkpoint_path" \
        --subset "validation_medium" \
        --batch-size 4 \
        --device "cuda" \
        --output-dir "$validation_dir" \
        --run-both-modes >> "$validation_log" 2>&1
    
    if [ $? -eq 0 ]; then
        # Generate comprehensive report with visualizations
        echo "Generating validation report..." >> "$validation_log"
        python scripts/generate_validation_comparison.py \
            --validation-dir "$validation_dir" \
            --output-path "${validation_dir}/validation_report.html" >> "$validation_log" 2>&1
        
        # Extract key metrics for notification
        if [ -f "${validation_dir}/validation_results_"*".json" ]; then
            local json_file=$(find "${validation_dir}" -name "validation_results_*.json" | head -1)
            local test_rmsd=$(grep -o '"test_equivalent".*"mean_rmsd":[^,]*' "$json_file" | grep -o '[0-9.]*')
            local test_tm=$(grep -o '"test_equivalent".*"mean_tm_score":[^,]*' "$json_file" | grep -o '[0-9.]*')
            
            notify "Comprehensive validation successful! Test-mode RMSD: ${test_rmsd:-N/A}, TM-score: ${test_tm:-N/A}" "success"
        else
            notify "Comprehensive validation completed but metrics unavailable" "warning"
        fi
    else
        notify "Comprehensive validation failed! Check logs for details." "error"
    fi
}
...
# 5. VALIDATION-ENHANCED: Added comprehensive dual-mode validation
...
python scripts/train_enhanced_model_fixed.py \
  ...
  --comprehensive_val_subset technical | tee -a "${LOG_DIR}/live_training.log" "${OUTPUT_DIR}/training.log" 2>&1 &
...
echo ""
echo "To view latest validation results:"
echo "  ls -la \$(find ${OUTPUT_DIR} -path '*/validation_results/*/*.html')"
echo ""
echo "To check validation metrics:"
echo "  cat \$(find ${OUTPUT_DIR} -path '*/validation_results/*/*.json' | sort | tail -1)"
echo ""
