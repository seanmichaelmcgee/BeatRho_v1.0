#!/bin/bash
# INTENSIVE PRODUCTION TRAINING - V4
# Enhanced training script with increased model size and compute intensity
# WARNING: This will use significantly more GPU resources than v3
# Builds on the successful device handling fixes in v3
...
# Run training with INTENSIFIED model and higher computational load
# 1. Larger model (more transformer blocks, larger dimensions)
# 2. More training epochs (500 vs 100)
# 3. More complex curriculum with longer sequences
...
# Begin intensive training - parameters increased ~5x
python scripts/train_enhanced_model_fixed.py \
  ...
  --ff_dim 1024 \
  --curriculum_learning \
  --curriculum_stages 50 100 150 200 250 300 350 400 450 500 \
  --batch_adaptive \
